...
export function SchemeFinder({ accessToken, onBack }: SchemeFinderProps) {
  ...
  const handleFindSchemes = async () => {
    ...
    try {
      // Call the backend API with user profile including location
      ...
      // Transform backend scheme data to frontend format
      const fetchedSchemes: Scheme[] = (data.schemes || []).map((scheme: any, index: number) => ({
        id: `scheme-${index}`,
        name: scheme.name || 'Unknown Scheme',
        description: scheme.description || 'No description available',
        eligibility: scheme.eligibility || [],  // Keep as array!
        benefits: scheme.benefits || [],  // Keep as array!
        applyLink: scheme.websiteLink || scheme.applicationProcess || '#',
        category: scheme.schemeType || 'General',  // Use schemeType from backend
        targetGroup: 'All Citizens' // Default value, can be enhanced based on eligibility
      }));

      if (fetchedSchemes.length === 0) {
        setError(`No schemes found for ${selectedState}${selectedDistrict ? `, ${selectedDistrict}` : ''}. The AI might need more specific information.`);
      }

      setSchemes(fetchedSchemes);
    } catch (error: any) {
      console.error('Schemes fetch error:', error);
      setError(error.message || 'Failed to fetch schemes from backend');
    } finally {
      setIsLoading(false);
    }
  };
                ...
                <Card key={scheme.id} className="hover:shadow-md transition-shadow">
                  <CardHeader>
                    <div className="flex items-start justify-between gap-4">
                      ...
                      <Badge className={scheme.category === 'State' ? 'bg-purple-100 text-purple-800' : 'bg-blue-100 text-blue-800'}>
                    ...
                    <div className="space-y-3">
                      <div>
                        <h4 className="text-sm font-semibold mb-2 flex items-center gap-2">
                          <span className="text-purple-600">✓</span> Eligibility:
                        </h4>
                        {typeof scheme.eligibility === 'string' ? (
                          <p className="text-sm text-gray-700 pl-5">{scheme.eligibility}</p>
                          ...
                          <p className="text-sm text-gray-500 italic pl-5">Not specified</p>
                        )}
                      </div>
                      <div>
                        <h4 className="text-sm font-semibold mb-2 flex items-center gap-2">
                        ...
                        {typeof scheme.benefits === 'string' ? (
                          <p className="text-sm text-gray-700 pl-5">{scheme.benefits}</p>
