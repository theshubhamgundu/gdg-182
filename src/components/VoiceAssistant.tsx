...
export function VoiceAssistant({ onVoiceCommand, currentScreen }: VoiceAssistantProps) {
  ...
  const handleVoiceCommand = async (command: string) => {
    ...
    console.log('🔍 Processing lowercase command:', lowerCommand);
    ...
    if (lowerCommand.includes('dashboard') || lowerCommand.includes('home') || lowerCommand.includes('डैशबोर्ड')) {
      ...
      console.log('✅ Matched SCHEMES command');
      ...
      console.log('✅ Matched COMPLAINTS command');
      ...
      console.log('✅ Matched PROFILE command');
      ...
      onVoiceCommand?.(commandType, selectedLanguage.code);
      console.log('📡 onVoiceCommand called');
    } else if (lowerCommand.includes('gram sabha') || lowerCommand.includes('village') || lowerCommand.includes('ग्राम सभा')) {
      console.log('✅ Matched GRAM SABHA command');
      response = getTranslatedResponse('Opening gram sabha', selectedLanguage.code);
      commandType = 'navigate:gram-sabha';
      isNavigation = true;
      console.log('🚀 Calling onVoiceCommand with:', commandType);
      onVoiceCommand?.(commandType, selectedLanguage.code);
      console.log('📡 onVoiceCommand called');
    } else if (lowerCommand.includes('chat') || lowerCommand.includes('companion') || lowerCommand.includes('साथी')) {
      console.log('✅ Matched COMPANION command');
      ...
      console.log('❓ No navigation match - treating as general query');
      // General query - use Groq-powered backend for intelligent response
      try {
        setIsListening(false); // Stop listening while processing

        // Use the new backend API with session management
        ...
        response = result.response || 'I am processing your request. Please ask again.';

        if (!response || response.trim().length === 0) {
          response = 'I am here to help. Please ask your question again.';
        }
      } catch (error: any) {
        console.error('Voice AI error:', error);
        response = `I encountered an issue. Please try again or rephrase your question.`;
      }
      commandType = `query:${command}`;
      console.log('🚀 Calling onVoiceCommand with query:', commandType);
      onVoiceCommand?.(commandType, selectedLanguage.code);
      console.log('📡 onVoiceCommand called for query');
    }
    ...
    if (!isNavigation) {
      console.log('🔄 Non-navigation command - will restart listening after response');
      setTimeout(() => {
        setTranscript('');
  ...
  const getTranslatedResponse = (text: string, langCode: string): string => {
    const translations: Record<string, Record<string, string>> = {
      'en': {
        ...
        'Opening gram sabha': 'Opening Gram Sabha summarizer',
      ...
      'hi': {
        ...
        'Opening gram sabha': 'ग्राम सभा सारांश खोल रहे हैं',
  ...
  const toggleListening = () => {
    try {
      if (isListening) {
        ...
        if (!recognitionRef.current) {
          alert('⚠️ Speech recognition not initialized. Please refresh the page.');
          return;
        }
      ...
      if (error.message.includes('already started')) {
        ...
        alert(`⚠️ Could not start voice recognition: ${error.message}\n\nTry:\n1. Refresh the page\n2. Allow microphone permissions\n3. Use Chrome or Edge browser`);
        setIsListening(false);
      }
    }
  };
