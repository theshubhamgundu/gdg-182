...
export function DocumentWalletEnhanced({ accessToken, onBack }: DocumentWalletEnhancedProps) {
  ...
  const fetchDocuments = async () => {
    try {
      const response = await apiFetch(`${API_BASE_URL}/documents/all`, {
        headers: {
  ...
  const fetchFamilyMembers = async () => {
    try {
      const response = await apiFetch(`${API_BASE_URL}/family`, {
        headers: {
          'Authorization': `Bearer ${accessToken}`
        }
      });
  ...
  const handleAddFamilyMember = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await apiFetch(`${API_BASE_URL}/family/add`, {
        method: 'POST',
        headers: {
  ...
  const handleRemoveFamilyMember = async (memberId: string) => {
    ...
    try {
      const response = await apiFetch(`${API_BASE_URL}/family/${memberId}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${accessToken}`
        }
